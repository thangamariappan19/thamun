...
//#region Post Method
  AnnouncementCreation(data: any) {
    ...
    return this.http.post(`${this.config.APIUrl}${this.config.Announcement}`, body, { headers: this.headers })
      .map(res => {
        return res.json();
      });
  };
  HomeworkCreation(data: any) {
    const body = JSON.stringify(data)
    return this.http.post(`${this.config.APIUrl}${this.config.Homework}`, body, { headers: this.headers })
      .map(res => {
        return res.json();
      });
  }; 
...
GetHomeworkFun(HomeworkId,SchoolId:any){
  const options = new RequestOptions({ headers: this.headers });
  return this.http.get(this.config.APIUrl + this.config.GetHomework + 'HomeworkId=' +HomeworkId + '&SchoolId=' + SchoolId , options).map((response: Response) => {
    return response.json();
  });
}
...
GetSearchAnnouncement(SchoolId,AffiliationMapId,announcementdate:any){
  const options = new RequestOptions({ headers: this.headers });
  return this.http.get(this.config.APIUrl + this.config.GetAnnouncementSearch + 'SchoolId=' +SchoolId +'&AffiliationMapId=' +AffiliationMapId +'&Date=' +announcementdate, options).map((response: Response) => {
    return response.json();
  });
}
  GetSearchHomework(SchoolId,AffiliationMapId,Grade,Section,Homeworkdate:any){
  const options = new RequestOptions({ headers: this.headers });
  return this.http.get(this.config.APIUrl + this.config.GetHomeworkSearch +'SchoolId=' +SchoolId +'&AffiliationMapId=' +AffiliationMapId+'&Grade=' +Grade +'&Section=' +Section +'&Date='+Homeworkdate, options).map((response: Response) => {
    return response.json();
  });
}
logout(data: any) {
  const body = JSON.stringify(data)
  return this.http.post(`${this.config.APIUrl}${this.config.LogOut}`, body, { headers: this.headers })
    .map(res => {
      return res.json();
    });
}; 
